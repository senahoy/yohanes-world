...
- **Career ladder rewards:** completing QA work promotes the visitor up a rank chip in the HUD, New Hire → Junior QA (first catch and filing) → QA Engineer (all six caught) → Senior QA (all filed plus CI automated) → QA Lead (incident resolved) → Island Legend (golden fish, full harvest, piano and dog on top of everything). Promotions ship a toast, a fanfare and confetti; the final rank plays a triple burst and its dialog funnels straight into the contact panel. Rank derives from the persisted save.
- **First-run tutorial:** new visitors get a two-line briefing dialog, then a guided mission: catch the bug near the landing pad, file its report at the kanban. A bottom-center MISSION banner with a rotating compass arrow and live distance leads the way; it clears after the first filing with a wrap-up that points at the minimap and teases the graveyard easter egg. Progress-derived, so returning players never see it.
...
- **Touch controls (coarse pointers):** always landscape — in portrait the page CSS-rotates 90° and plays immediately (renderer dimensions swap, joystick coordinates remap), with a transient 3.5s rotate hint; Android additionally gets fullscreen + orientation lock on entry. Screen splits at the midline: left half is the movement joystick, right half is one-finger camera orbit, and the right thumb also gets pixel-labeled `JUMP` and `ACT` buttons (≥64px, no emoji glyphs) — everything responds on touchstart with independent touch IDs, so steering, looking and jumping all work simultaneously.
